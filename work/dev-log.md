...
| B1(部分) | 2026-08-23 | PKG-13(全)、PKG-08 L-5/L-6(auth+SSE+schema)、PKG-09 A-2/A-3/A-4/A-6(App 壳+登录对接+分级+组件库) | 会话台账待回填 | — | 约 1,900(累计) | 修复后通过:server lint/test(52)/build 绿;client 编译 0 error + EditMode 30/30 | 待签 | B1 剩余:迁移执行(Docker 阻断)、L-2 业务表补全、真机走查;同日修 Conv 常量与 3 个编译错 |
